...
describe("ElectronProtocol", () => {
  ...
  it("exposes fork protocol schemes distinct from official t3code", () => {
    assert.equal(ElectronProtocol.DESKTOP_PRODUCTION_SCHEME, "t3code-pi");
    assert.equal(ElectronProtocol.DESKTOP_DEVELOPMENT_SCHEME, "t3code-pi-dev");
    assert.equal(ElectronProtocol.getDesktopScheme(false), "t3code-pi");
    assert.equal(ElectronProtocol.getDesktopScheme(true), "t3code-pi-dev");
    assert.equal(ElectronProtocol.getDesktopOrigin(false), "t3code-pi://app");
    assert.equal(ElectronProtocol.getDesktopUrl(true), "t3code-pi-dev://app/");
  });
  ...
  it.effect("proxies the stable renderer origin to the current app server", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const response = yield* Effect.promise(() =>
            handler!(
              new Request("t3code-pi-dev://app/api/health?verbose=1", {
                headers: {
                  accept: "application/json",
                  origin: "t3code-pi-dev://app",
                  referer: "t3code-pi-dev://app/",
          ...
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "img-src 'self' t3code-pi-dev: blob: data: http: https:",
          );
          assert.include(
            response.headers.get("content-security-policy") ?? "",
            "font-src 'self' t3code-pi-dev: data:",
          );
        }),
      );
      ...
      assert.deepEqual(unhandleMock.mock.calls, [["t3code-pi-dev"]]);
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("t3code-pi://other/")));
        }),
      );
    ...
    Effect.gen(function* () {
      ...
      const response = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          return yield* Effect.promise(() => handler!(new Request("t3code-pi-dev://app/")));
        }),
      );
  ...
  it.effect("preserves protocol registration failures", () =>
    Effect.gen(function* () {
      ...
      assert.equal(error.scheme, "t3code-pi-dev");
      assert.strictEqual(error.cause, cause);
      assert.equal(error.message, 'Failed to register Electron protocol scheme "t3code-pi-dev".');
    }).pipe(Effect.provide(ElectronProtocol.layer)),
  );
  ...
  it.effect("preserves protocol unregistration failures", () =>
    Effect.gen(function* () {
      ...
      if (exit._tag === "Failure") {
        ...
        assert.equal(error.scheme, "t3code-pi");
        assert.strictEqual(error.cause, cause);
        assert.equal(error.message, 'Failed to unregister Electron protocol scheme "t3code-pi".');
      }
    }).pipe(Effect.provide(ElectronProtocol.layer)),
  );
  ...
  it("keeps executable sources host-restricted while allowing runtime network resources", () => {
    ...
    assert.deepEqual(directives["font-src"], ["'self'", "t3code-pi:", "data:"]);
  });
});
